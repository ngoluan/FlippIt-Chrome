...
function sendOpenMessage(element) {
    ...
    if (app === true && historyData[index].fileName !== "") {
        chrome.tabs.create({
            url: "http://www.flippit.ca/index.html?msgID=" + id
        });
    }
    else if (app === false && historyData[index].fileName !== ""){
        window.open("http://www.flippit.ca/uploads/"+localStorage.email+"/"+historyData[index].fileName);
    }
}
