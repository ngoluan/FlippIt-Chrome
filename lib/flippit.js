...
function init() {
    ...
    $(document).bind('ready', function() {
        if (app === true) {
            ...
            $('.page').css('min-height', ($(window).height() - $('#header').height() - $('.footer').height() - 16));
        }
...
function showSend() {
    ...
    dropZone = new Dropzone("div#message", {
        url: serverPath + "/pass/server/upload_v2.php",
        maxFiles: 1,
    ...
    $.post(query, postData, function(data) {
        ...
        $.each(data, function(index, historyItem) {
            ...
            if (typeOfMessage(historyItem.fileName) == "image") {
                ...
                buttons += '<div class="button download" data-passIndex="' + index + '"></div>';
                buttons += '<div class="button open" data-passIndex="' + index + '"></div>';
            } else if (typeOfMessage(historyItem.fileName) == "file") {
                buttons += '<div class="button download" data-passIndex="' + index + '"></div>';
                buttons += '<div class="button open" data-passIndex="' + index + '"></div>';
            } else if (typeOfMessage(historyItem.fileName) == "text") {
                buttons += '<div class="button copy" data-passIndex="' + index + '"></div>';
            }

            buttons += '<div class="button send" data-passIndex="' + index + '"></div>';
            buttons += '<div class="button delete" data-passIndex="' + index + '"></div>';
...
function downloadHistory(element) {
    ...
    chrome.downloads.download({
        url: "http://www.local-motion.ca/pass/uploads/" + email + "/" + fileName
    });
}
...
function send(message) {
    ...
    $.each(storedDevices, function(index, device) {
        var targetType = "";
        query = serverPath + "/pass/server/send_v2.php";
...
function showDevices() {
    ...
    $('html,body').height($('.page').height() + $('#header').height());
}
...
function copyMessage(element) {
    var index = element.attr('data-passIndex');
    /* 	console.log(index);
     message = historyData[index].message;

     $("#clipboard").val(message);
     var copyDiv = document.getElementById('clipboard');
     copyDiv.focus();
     copyDiv.select(); */
    var message = $('.button.copy[data-passindex="'+index+'"]').parent().parent().find('.message');
...
function changeDeviceName(deviceName, targetID) {
    var query, postData;
    $.mobile.loading('show');
    query = serverPath + "/pass/server/changeDeviceName_v1.php";
    postData = {
        targetID: targetID,
...
function deleteMessage(element) {
    ...
    console.log(index);
    var query, postData;
    query = serverPath + "/pass/server/deleteMessage_v1.php";
    postData = {
        id: id,
        email: localStorage.email
    };
