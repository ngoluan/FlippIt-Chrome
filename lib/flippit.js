...
function init(){
	...
	$(document).bind('ready', function() {
        pageShowInit();
		if(checkUser()===false){loginPageInit();}else{sendPageInit();}
	});
}
...
function historyPageInit(totalLoad){
	$.mobile.loading('show');
	$( ":mobile-pagecontainer" ).pagecontainer( "change", "#historyPage");
	
	var query, postData;
    if (totalLoad === "") {totalLoad = 20;}
        ...
        if(page=="sendPage"){
            ...
            if(dropZone===null){
                dropZone = new Dropzone("div#message", {
                    ...
                    dragover: function() {$('#message').css('background', '#FFF954');},
                    dragleave: function() {$('#message').css('background', '#ffffff');},
                    drop: function(e) {
                        if ($('#message').text() == "Type message or drag and drop file here") {$('#message').text("");$('#message').css('background', '#ffffff');}
                    },
                    ...
                    init: function() {
                        ...
                        this.on("sending", function(file, xhr, formData) {
                            ...
                            if (typeof localStorage.savedTargetDevice != "undefined") {savedTargetDevice = $.parseJSON(localStorage.savedTargetDevice);}
...
function checkUser(){
	if(typeof localStorage.email =="undefined"){return false;}
}
function showDevices() {
    ...
    if (typeof localStorage.savedTargetDevice != "undefined") {savedTargetDevice = $.parseJSON(localStorage.savedTargetDevice);}
	if (typeof localStorage.devices != "undefined") {devices = $.parseJSON(localStorage.devices);}else{getDevices(); return true;}
    ...
    $.each(devices, function(index, device) {
        ...
        var image = "";
        var deviceName = "";
		if (savedTargetDevice.targetID == device.targetID) {active = "active";}

        if (device.type == "chrome") {image = "computer_black.png";} 
		else if (device.type == "android") {image = "phone_black.png";} 
		else if (device.type == "blackberry") {image = "phone_black.png";} 
		else if (device.type == "cloud") {image = "cloud_black.png";}
		
        if (typeof localStorage.regID != "undefined") {
            if (localStorage.regID == device.targetID) {return true;}
        }
		
        i++;
		
        if (device.name !== "") {deviceName = device.name;}
		else{deviceName =device.type;}
    ...
    if (slider === null) {
		var maxSlides;
		if(app===false){maxSlides=6;}else{maxSlides=3;}
...
function send(message) {
	...
	if(savedTargetDevice.type=="chrome"){localSaveMessage="true";}
	else{localSaveMessage=saveMessage;}
	...
	$.post(query, postData, function(data) {
		data = $.parseJSON(data);
		if (typeof data.error != "undefined") {toast(data.error, "short");}
		else{toast("Message sent", "short");}
		$.mobile.loading('hide');
	}).fail(function(xhr, status, error) {
		toast("Something wrong with getting data from the server.", "short");
		$.mobile.loading('hide');
...
function toast(msg, length){
	...
	var delay;
	if(length=="long"){delay=6000;}else{delay=3000;}
...
function setupEvents(){
	$(document).on("tap", '#loginInputBtn', function(e) {login();});
	$(document).on("tap", '#regInputBtn', function(e) {register();});
	...
	$(document).on("tap", '#message', function(e) {clearMessagePlaceholder($(this));});
		$(document).on("tap", '#saveMessageBtn', function(e) {setSaveMessage($(this));});
		$(document).on("tap", '#clear', function(e) {$('#message').html("");dropZone.removeAllFiles();});
	...
	$(document).on("tap", '.header .button.history', function(e) {historyPageInit(20);});
	...
	$(document).on("tap", '.help', function(e) {openHelp();});
